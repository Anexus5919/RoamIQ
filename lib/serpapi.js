...
export async function searchLocalPlaces({ destination, query = 'best restaurants', limit = 6 }) {
  ...
  return results.slice(0, limit).map((r) => ({
    ...
    // Every google_local result carries coordinates, which is what lets the
    // planner group nearby places onto the same day.
    coords: Number.isFinite(r.gps_coordinates?.latitude)
      ? { lat: r.gps_coordinates.latitude, lon: r.gps_coordinates.longitude }
      : null,
  }));
}
